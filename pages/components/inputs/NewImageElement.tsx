...
import { ImageElementProps, ImageProps } from '@/types/images';
import { LargeHorizontal, LargeVertical,VerticalDiptych,HorizontalDiptych,VerticalTriptych,HorizontalStack } from './ImageInputs/imageUploaders';
const NewImageElement: React.FC<ImageElementProps> = ({ index, onUpload, element }) => {
  const [subType, setSubType] = useState<string>(element?.subType || 'lh');
  const [imageUrl, setImageUrl] = useState<string[]>([]);

  const layouts: Record<string, React.FC<ImageProps>> = {
    lh: LargeHorizontal,
    lv: LargeVertical,
    vd: VerticalDiptych,
  ...
  return (
    <div style={{ minWidth: '55vw', maxWidth: '55vw' }}>
      ...
      <LayoutComponent index={index} 
                       onUpload={onUpload} 
                       urls={(element?.imageUrls || imageUrl).map((u) =>
                               typeof u === 'string' ? u : u.imageUrl
                              )
                            }/>
    </div>
  );
...
//   index: number;
//   onUpload: (args: { index: number; url: string; subType?: string; position?: string }) => void;
//   element: BlogElement;
// }
...
//   );
// };
// const LargeHorizontal: React.FC<ImageProps> = ({index, onUpload, urls} ) => {
...
//   );
// };
// const LargeVertical: React.FC<ImageProps> = ({index, onUpload, urls} ) => {
//   const [imageUrl, setImageUrl] = useState(urls?.[0] || null);
...
//   );
// };
// const VerticalDiptych: React.FC<ImageProps> = ({index, onUpload, urls} ) => {
...
//   const handleImageUpload2 = (url: string) => {
//     setImageUrl2(url);
...
//   );
// };
// const VerticalTriptych: React.FC<ImageProps> = ({index, onUpload, urls} ) => {
//   const [imageUrl1, setImageUrl1] = useState(urls?.[0] || null);
//   const [imageUrl2, setImageUrl2] = useState(urls?.[1] || null);
//   const [imageUrl3, setImageUrl3] = useState(urls?.[2] || null);

//   const handleImageUpload1 = (url: string) => {
//     setImageUrl1(url);
//     onUpload(url); 
//   };
...
//         <ImageInput onUpload={handleImageUpload1} type='smallV'/>
//       ) : (
//         <ImagePreview imageUrl={imageUrl1}  classes="w-1/3 w-48 object-cover" width={600} height={600}/>
//       )}
      
//       {!imageUrl2 ? (
//         <ImageInput onUpload={handleImageUpload2} type='smallV'/>
//       ) : (
//         <ImagePreview imageUrl={imageUrl2}  classes="w-1/3 w-48 object-cover" width={600} height={600}/>
//       )}
      
//       {!imageUrl3 ? (
//         <ImageInput onUpload={handleImageUpload3} type='smallV' />
//       ) : (
//         <ImagePreview imageUrl={imageUrl3}  classes="w-1/3 w-48 object-cover" width={600} height={600}/>
//       )}
//     </div>
//   );
// };
// const HorizontalDiptych: React.FC<ImageProps> = ({index, onUpload, urls} ) => {
//   const [imageUrl1, setImageUrl1] = useState(urls?.[0] || null);
//   const [imageUrl2, setImageUrl2] = useState(urls?.[1] || null);
...
//   );
// };
// const HorizontalStack: React.FC<ImageProps> = ({index, onUpload, urls} ) => {
//   const [imageUrl1, setImageUrl1] = useState(urls?.[0] || null);
//   const [imageUrl2, setImageUrl2] = useState(urls?.[1] || null);
